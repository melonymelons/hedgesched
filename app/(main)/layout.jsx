...
const navItems = [
    { href: "/dashboard", label: "仪表板", icon: BarChart },
    { href: "/events", label: "活动", icon: Calendar },
    { href: "/meetings", label: "会议", icon: Users },
    { href: "/availability", label: "可用时间", icon: Clock },
  ];  
        ...
        <div className = "flex flex-col h-screen bg-red-50 md:flex-row">
            ...
            <header className = "flex justify-between items-center mb-4">
                <h2 className = "text 5xl md:text-6xl gradient-title pt-2 md:pt-0 text-center md:text-left w-full">
                    {navItems.find((item) => item.href === pathname).label ||"仪表板"}
                </h2> 
            </header>
            {children}
        </main>
            <nav className = "md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-md">
                <ul className = "flex justify-around">
                        {navItems.map((item) => (
                            <li key = {item.href} >
                                 <Link href = {item.href} className = {`flex flex-col items-center py-2 px-4 ${
